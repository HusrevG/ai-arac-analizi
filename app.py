...
if "ilan" in query_params:
    otomatik_link = query_params["ilan"]
...
with col1:
    yil = st.number_input(
        ...
        min_value=1990,
        max_value=2026,
        value=st.session_state.get("yil", 2020)
    )
...
with col2:
    km = st.number_input(
        ...
        min_value=0,
        max_value=1000000,
        value=st.session_state.get("km", 100000),
        step=1000
    )
...
vites = st.selectbox(
    ...
    index=["Otomatik", "Manuel", "Yarı Otomatik"].index(
        st.session_state.get("vites", "Otomatik")
    )
)
...
boya = st.number_input("Boya Sayısı", min_value=0, max_value=20, value=0)
degisen = st.number_input("Değişen Sayısı", min_value=0, max_value=20, value=0)
